...
void main() {
  group('MockServer', () {
    ...
    test('can add and retrieve a release with custom created_at', () async {
      ...
      final release = Release.new_('octocat', 'hello-world', 'v1.1.0')
      ...
      final client = HttpClient();
      addTearDown(client.close);
      final request = await client.getUrl(Uri.parse('$uri/repos/octocat/hello-world/releases/tags/v1.1.0'));
      final response = await request.close();
      final body = await response.transform(utf8.decoder).join();

      expect(response.statusCode, equals(200));
      final json = jsonDecode(body) as Map<String, dynamic>;
      expect(json['tag_name'], equals('v1.1.0'));
      expect(json['created_at'], equals(timestamp));
      expect(json['published_at'], equals(timestamp));
    });
  });
}
